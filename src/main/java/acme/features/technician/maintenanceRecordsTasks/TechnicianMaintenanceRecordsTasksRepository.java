...
public interface TechnicianMaintenanceRecordsTasksRepository extends AbstractRepository {
	...
	@Query("select t from Task t where t not in (select mrt.task from MaintenanceRecordsTasks mrt where mrt.maintenanceRecord.id = :maintenanceRecordId)")
	Collection<Task> findTasks(int maintenanceRecordId);
